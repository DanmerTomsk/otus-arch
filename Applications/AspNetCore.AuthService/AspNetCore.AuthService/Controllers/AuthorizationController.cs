...
namespace AspNetCore.AuthService.Controllers
{
    [Route("auth")]
    [ApiController]
    public sealed class AuthorizationController : ControllerBase
    {
        ...
        {
            var user = await _accountsDbContext.Users.Include(user => user.Role).FirstOrDefaultAsync(x => x.Username == credentials.Username);

            if (user is null || !PasswordHasher.VerifyHash(credentials.Password, user.PasswordHash))
            {
